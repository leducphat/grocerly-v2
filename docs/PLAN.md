...
| P-12 | Cài pytest + pytest-django, viết test cho luồng cốt lõi | G5, TC2.5 | Đang làm | Ưu tiên: auth, giỏ hàng, checkout, tool AI. **16/09/2026:** dựng hạ tầng test theo D-018 (`settings_test.py`, `pytest.ini`, `conftest.py`, `requirements-dev.txt`); 48 test cho đăng ký/đăng nhập, giỏ hàng, checkout, VNPay, trợ lý AI — 43 xanh, 5 `xfail` tái hiện lỗi L-6 → L-10 (3 lỗi mới L-8, L-9, L-10 phát hiện khi viết test). **23/09/2026:** thêm `useradmin/tests/` (UC-20, sáu test) và `core/tests/test_reviews.py` (UC-14, chín test) — tổng 79 test, không có `xfail`. Còn: `useradmin` phần sản phẩm và thống kê (UC-19, UC-26), wishlist, hồ sơ; mỗi AC viết ở P-23 cần test tương ứng |
| P-13 | Báo cáo coverage trong CI | TC2.5 | Đang làm | Mức 5 cần ≥ 70% ở module cốt lõi. Đã cài pytest-cov (D-018). Số đo đầu tiên, chạy tay 16/09/2026: tổng 66%; `core/views.py` 57%, `store_api/views.py` 73%, `userauths/views.py` 71%, `useradmin/views.py` 21%. **17/09/2026:** CI in bảng độ phủ vào trang tóm tắt mỗi lần chạy và lưu `coverage.xml` (D-019). **23/09/2026:** sau L-3 và L-5, tổng lên 71%; `core/views.py` 65%, `store_api/views.py` 78%, `userauths/views.py` 71%, `useradmin/views.py` 32%. **24/09/2026:** 70% sau khi sửa lint, vì các hàm `__str__` mới thêm chưa có test gọi tới. Còn: chốt "module cốt lõi" trong bản cam kết P-03; cân nhắc đặt ngưỡng tối thiểu làm CI đỏ khi đã chốt |
| P-14 | Cấu hình linter + phân tích tĩnh, đưa vào CI | TC2.4 | **Xong** 24/09/2026 | Mức 5 cần 0 lỗi lint, 0 issue Blocker/Critical, trùng lặp ≤ 3%. Ruff 0.16.8 làm cả linter lẫn công cụ phân tích tĩnh (D-029), jscpd 5.2.1 đo trùng lặp trên mã Python (D-031); cả hai chạy trong job `lint`. Lần chạy đầu 30 lỗi lint, khoảng 7 trên 1000 dòng; sửa hết cùng ngày. Số đo: 0 lỗi lint, 0 Blocker/Critical, trùng lặp 1,09%. Cách quy đổi Blocker/Critical và phạm vi đo trùng lặp ở `COMMITMENT.md` §5.1, chờ GVHD duyệt cùng bản cam kết (P-03). Xóa 13 template mẫu của theme mà không view nào dùng, khoảng 22.400 dòng; template còn lại trùng 26,6%, ghi trong bản cam kết là hạn chế |
| P-15 | Quét secret tự động (gitleaks) trong CI | TC2.4 | **Xong** 24/09/2026 | Theo D-030: job `secret-scan` quét toàn bộ lịch sử Git mỗi lần CI chạy, lưu `gitleaks-report.json` làm artifact. Chạy thử trên bản clone mới từ `origin`: 169 commit, 0 phát hiện. Ở máy ra thêm 2 phát hiện giả từ tag lưu trữ chỉ có ở remote `tlcn` (xem D-030) |
| P-16 | Health check + log tập trung + cảnh báo sau triển khai | TC2.6 | Chưa bắt đầu | Yêu cầu của Mức 5 |
| P-17 | Sửa các lỗi nghiệp vụ và bảo mật đã phát hiện (L-3, L-5 → L-10 trong [`COMMITMENT.md`](COMMITMENT.md) §3) | TC2.2, TC2.5 | **Xong** 23/09/2026 | Mỗi lỗi có sẵn test tái hiện đánh dấu `xfail` từ P-12; sửa xong thì gỡ `xfail` trong cùng commit. **21/09/2026:** xong L-6 — giá giỏ hàng và tổng đơn đọc từ `Product` thay vì từ tham số trình duyệt, tiền tính bằng `Decimal` (D-022); thêm 3 test cho hành vi mới. Cùng ngày xong L-8 — trang hoàn tất không còn tự đặt đơn thành đã thanh toán, chỉ `vnpay_return` / `vnpay_ipn` làm việc đó (D-023); thêm 2 test. Xong L-7 — `_refresh_cart` hạ số lượng của dòng giỏ xuống bằng tồn kho và bỏ dòng hết hàng (D-024); thêm 5 test. **22/09/2026:** xong L-10 — công cụ AI và `/api/v1/products/` lọc theo `product_status` như các trang cửa hàng, thay cho hai cờ `status` và `in_stock` mà không form nào ghi (D-025); xong L-9 — AI từ chối sản phẩm hết hàng và nói rõ lý do cho Gemini. Bộ test hết `xfail`. **23/09/2026:** xong L-5 — `change_order_status` từ chối trạng thái ngoài `STATUS_CHOICES` và không cho đổi đơn đã giao (D-026); xong L-3 — chỉ khách có đơn đã rời kho mới đánh giá được, mỗi người một lần (D-027). Không còn lỗi nào phải sửa code; còn L-2 và L-4 chốt theo hướng sửa đặc tả |
| P-18 | Sửa L-11: đơn quay lại `shipped` bị trừ tồn kho lần nữa | TC2.2, TC2.5 | Chưa bắt đầu | Phát hiện 24/09/2026 khi review PR #8. Test tái hiện đã có trong `useradmin/tests/test_orders.py`, đánh dấu `xfail`. Cần chốt trước: hoàn kho khi đơn rời `shipped`, hay mỗi đơn chỉ trừ kho một lần (thêm trường cho `CartOrder`, cần migration). Sửa xong thì gỡ `xfail` trong cùng commit |
...
| 9 | 22/09/2026 | SRS §6.1 | Thêm ràng buộc cho UC-16 và UC-17: trợ lý AI và API `/api/v1/products/` chỉ thấy đúng tập sản phẩm khách thấy ở cửa hàng. Viết rõ thêm ở UC-17 là AI phải nói lý do hết hàng | Sửa L-10 (D-025). Đặc tả TLCN chưa bao giờ nói AI nhìn thấy gì, nên không có câu nào để đối chiếu khi công cụ AI lọc theo một cột khác với cửa hàng — viết ra thì chỗ lệch lộ ngay | `6b7dec8` |
| 10 | 23/09/2026 | SRS §6.1 | UC-20: thêm ràng buộc máy chủ chỉ nhận ba trạng thái `Processing`, `Shipped`, `Delivered`; giá trị khác bị từ chối | Sửa L-5 (D-026). Câu cũ chỉ nói đơn đã giao thì không đổi nữa, không nói gì về tập giá trị hợp lệ — mà ô chọn trên giao diện đang gửi được một trạng thái không tồn tại | `9316171` |
| 11 | 23/09/2026 | SRS §6.1 | UC-14: điều kiện đánh giá đổi từ đơn `Shipped` sang đơn `Shipped` hoặc `Delivered`; thêm giới hạn mỗi khách một đánh giá cho một sản phẩm; nói rõ máy chủ từ chối chứ không chỉ giao diện ẩn nút | Sửa L-3 (D-027). Theo câu cũ thì khách nhận được hàng rồi lại mất quyền đánh giá, vì đơn lúc đó đã sang `Delivered`. Giới hạn một lần vốn là hành vi của template nhưng chưa ai viết ra thành yêu cầu | `2d0bf9c` |
| 12 | 23/09/2026 | SRS §4.4 (FR-A-02), §6.1 | Bỏ bước Admin duyệt sản phẩm: sản phẩm nhân viên tạo hiển thị ngay, Admin vẫn ẩn hoặc gỡ bán được | Chốt L-2 theo hướng sửa đặc tả (D-028). Bước duyệt sinh ra cho mô hình nhiều người bán mà D-015 đã bỏ; nhân viên cửa hàng duyệt hàng của chính mình thì không có nghĩa | `7d1bb3c` |
| 13 | 23/09/2026 | SRS §6 (UC-14), §6.1 | Bỏ chữ "(CRUD)" ở UC-14; viết rõ khách chỉ tạo đánh giá, quản trị viên kiểm duyệt | Chốt L-4 theo hướng sửa đặc tả (D-028). Cam kết CRUD đầy đủ cho người viết là tự thêm chức năng vào mẫu số của TC2.2 mà không định làm | `7d1bb3c` |

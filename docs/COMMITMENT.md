...
Đã chốt: L-1 (sửa đặc tả), L-6, L-7 và L-8 (sửa code, 21/09/2026). Nếu các chỗ
còn lại chốt theo đề xuất: thêm **2 lần sửa đặc tả** (L-2, L-4 — ghi vào
`PLAN.md` §6) và **4 lần sửa code** (L-3, L-5, L-9, L-10), mỗi lần có test đi kèm.
L-9 và L-10 vẫn còn test tái hiện lỗi đánh dấu `xfail` (D-018); sửa xong lỗi nào
thì gỡ `xfail` của lỗi đó trong cùng commit.
